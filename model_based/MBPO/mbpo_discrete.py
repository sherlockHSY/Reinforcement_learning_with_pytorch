...
class SAC_discrete:
    ''' 处理离散动作的SAC算法 '''
    def __init__(self, state_dim, action_dim, hidden_dim_1, hidden_dim_2, actor_lr, critic_lr,
                 alpha_lr, target_entropy, tau, gamma, device):
        # 策略网络
        self.actor = PolicyNet(state_dim, hidden_dim_1, hidden_dim_2, action_dim).to(device)
        # 第一个Q网络
    ...
    def __init__(self,
        ...
        self._min_logvar = nn.Parameter((-torch.ones(
            (1, self._output_dim // 2)).float() * 10).to(device),
                                        requires_grad=False)
        # 输入是状态和动作
        self.layer1 = FCLayer(self._input_dim, 200, ensemble_size, Swish(), device)
...
class MBPO_Discrete:
    ...
    def __init__(self, state_dim, action_dim, config, device):
        ...
        # self.env = env
        self.agent = SAC_discrete(state_dim, action_dim, self.hidden_dim_1, self.hidden_dim_2, self.lr_actor,
    ...
    def train_model(self):
        ...
        action_1 = np.array(action, dtype=np.float32)
        action_1 = action_1.reshape((action_1.shape[0],1))
        inputs = np.concatenate((obs, action_1), axis=-1)
...
if __name__ == '__main__':
    ...
    mbpo = MBPO_Discrete(state_dim, action_dim, config, device)
    return_list = []
    explore_return = mbpo.explore(env)  # 随机探索采取数据
    print('episode: 1, return: %d' % explore_return)
    return_list.append(explore_return)

    for i_episode in range(num_episode - 1):
        obs, done, episode_return = env.reset(), False, 0
        step = 0
        while not done:
            if step % 50 == 0:
